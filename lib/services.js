...
if (IS_BROWSER) {
 ...
 //socket = socketIOClient('https://voipserver-greatsun.herokuapp.com/', { transports: ['websocket'], jsonp: false });
...
//------------------------ MESS FUNCTION----------
function SendMess(fromNo,toNo,mess){
    socket.emit("MessSend",{fromNo,toNo,mess})
}

function MessSavedNotice(){
  socket.emit("MessSaved")
}

//waitforCall
function waitforMess(callback) {
  socket.on('MessComming', data => callback(data));
}
